...
namespace Lemoo.App.Controls.Tabs;
...
public partial class DocumentTabHost : UserControl, INotifyPropertyChanged
{
    ...
    private void UpdateFullscreenState()
    {
        ...
        if (_isFullscreen)
        {
            // Hide sidebar and expand content to full window
            var sidebar = mainWindow.FindName("Sidebar") as UIElement;
            if (sidebar != null) sidebar.Visibility = Visibility.Collapsed;
            // remove margins around content area (assumes DocumentTabHost is in a Grid with Margin)
        ...
        {
            pageIcon.Text = _isFullscreen ? "\uE73F" : "\uE740"; // toggle icon
        }
    }
    ...
    private void ToggleWindowFullscreen()
    {
        ...
        if (_isWindowFullscreen)
        {
            ...
            // enter fullscreen: hide chrome and sidebar, remove margin, make topmost and maximize
            var sidebar = mainWindow.FindName("Sidebar") as UIElement;
            if (sidebar != null) sidebar.Visibility = Visibility.Collapsed;
            ...
            // hide the title row so there's no empty space at top
    ...
    /// </summary>
    public void OpenPage(string title, System.Windows.Controls.Page page, string pageKey)
    {
        // 使用字典快速查找标签页（O(1) 而不是 O(n)）
...
public class DocumentTab : INotifyPropertyChanged
{
    ...
    public string PageKey { get; set; } // 用于标识页面，判断是否已打开
    ...
    public DocumentTab(string title, System.Windows.Controls.Page? page, string pageKey)
    {
        Title = title;
        Page = page;
        PageKey = pageKey;
